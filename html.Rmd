...
En la figura se puede observar el largo de pico en mm con respecto al ancho de pico en mm, se puede evidenciar que para Adelie en la isla Biscoe a medida que crece el largo de pico su anchura tambien, sin embargo se evidencia una diferencia de estas variables en la isla Torgersen en donde el largo de pico es mas pequeño pero su anchura es mayor. Por otro lado, se teiene a Chrinstrap el cual presenta datos unicamente para la isla Dream teniendo un valor de R de 0.43 y su correlación de datos es mayor. Finalmente, se tiene para Gentoo el cual presenta datos solo en la isla Biscoe se puede evidencia que sus datos están correlacionados. 
Por otra parte, se puede ver en la gráfica de caja y bigote los datos a través de cuartiles en donde se analiza el ancho de pico con respecto a las especies, tanto para Adelie y Chinstrap el ancho de pico es similar con respecto a Gentoo, asimismo se evidencia una cierta variabilidad fuera de los cuartiles. 
...
P4= ggplot(penguins, aes(x=Body_Mass_g, y = Flipper_Length_mm,  color=factor(species))) + geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) + geom_point(size=0.6, alpha=0.2) +  theme_classic() + facet_grid(factor(island)~factor(sex)) +  theme(legend.position = "none") + 
stat_poly_eq(coef.digits = 2,use_label(c("eq", "adj.R2")), formula = y ~ poly(x,1, raw = TRUE), label.x.npc = 0.9, label.y.npc = 1.0, cex=3) + labs(x = "Body Mass [mm]", y = "Flipper Length [mm]")
...
**INTERPRETACIÓN:** 
En el mapa de calor se evidencia que ha mayor distancia la intensidad del color aumenta por ejemplo a una distancia de 150 se puede ver un color más amarillento y asimismo el tipo de suero que se emplea es el HIHS y el tiempo es de 48 H. Por otro lado, tambien se puede evidenciar que para las intensidades de color mas bajas el tipo de suero emplado es HS y el tiempo de exposicion al tratamiento para x10, x3 y x17 es de 48 H sin embargo para x2, x16 el tiempo de es 24 H. 
